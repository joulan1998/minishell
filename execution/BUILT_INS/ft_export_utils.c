...
/*   Updated: 2024/09/21 15:04:17 by ael-garr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_syntax_for_export(char *string)
{
	...
	{	
		...
		{
			if (!ft_isalpha(local[i]) && !ft_isdigit(local[i]) && local[i] != '_')
				return(free(local),0);
			i++;
		}
		if (ft_isalpha(local[i]) || ft_isdigit(local[i]) || local[i] == '+' || local[i] == '_')
			return(free(local),1);
	}
	return(free(local),0);
}
